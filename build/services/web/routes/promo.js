...
router.get('/:id(\\d+)/rate', (0, _expressMysqlHelpers.procedureApi)({
  query: 'SELECT * FROM promo_rate where promo_id = ?',
  paramsHandler: ({
    id
  }) => [id]
}));
router.get('/:id(\\d+)/rate/history', (0, _expressMysqlHelpers.procedureApi)({
...
router.get('/:id(\\d+)/:domain(/^[A-Za-z]+$/)', (0, _expressMysqlHelpers.getApi)({
  query: ({
    id,
    domain
  }) => `SELECT * FROM ${domain}_promo WHERE promo_id = ${id}`
}));
router.post('/:promoId(\\d+)/:domain(/^[A-Za-z]+$/)/:domainId(\\d+)', (0, _expressMysqlHelpers.procedureApi)({
...
router.put('/:id(\\d+)/:domain(/^[A-Za-z]+$/)', (0, _expressMysqlHelpers.procedureApi)({
...
router.delete('/:id(\\d+)/:domain(/^[A-Za-z]+$/)', (0, _expressMysqlHelpers.procedureApi)({
